...
function drawBarChart(stuff1, stuff2, stuff3){
...
let numCng = parseInt(stuff2.space,10);
...
let space = numCng;
    ...
    for(let i = 0; i < stuff1.length; i++){
        ...
        if(i === 0){
        let bar = $("<div></div>");
        $(bar).css("gridArea", (201 - Math.trunc((stuff1[i][0] * multiplier))) + " / " + (i+1) + " / " + 201 + " / " + (i+2) );
        $(bar).css("backgroundColor", stuff1[i][1]);


        let derp = $("<div></div>");
        if(stuff2.posOfLabel == "bottom"){
        $(derp).css("gridArea", 190 + " / " + (i+1) + " / " + 201 + " / " + (i+2) );
        $(derp).html(stuff1[i][0]);
        $(chart).append(bar);
        $(chart).append(derp);
       } else if(stuff2.posOfLabel == "middle"){
           $(derp).css("gridArea", 195 - (Math.trunc((stuff1[i][0] * multiplier))/2)
            + " / " + (i+1) + " / " + (201 - (Math.trunc((stuff1[i][0] * multiplier)))/2)  +  " / " + (i+2) );
           $(derp).html(stuff1[i][0]);
           $(chart).append(bar);
           $(chart).append(derp);
         }
       else{ //top
           $(bar).html(stuff1[i][0]); //automatically to top
           $(chart).append(bar);
         }

       } else if(i !== 0){
        let bar = $("<div></div>");
        $(bar).css("gridArea", (201 - Math.trunc((stuff1[i][0] * multiplier))) + " / " + (i + 1 +  space) + " / " + 201 + " / " + (i + 2 + space) );
        $(bar).css("backgroundColor", stuff1[i][1]);
        ...
        $(chart).append(derp);
           ...
           $(derp).css("gridArea", 195 - (Math.trunc((stuff1[i][0] * multiplier))/2)
            + " / " + (i + 1 +  space) + " / " + (201 - (Math.trunc((stuff1[i][0] * multiplier)))/2)  +  " / " + (i + 1 +  space) );
           $(derp).html(stuff1[i][0]);
           $(chart).append(bar);
           $(chart).append(derp);
         }
...
//yAxis
let yAxis = $("<div></div>");
$(yAxis).css("height", stuff2.heightOf);
$(yAxis).css("width", stuff2.widthOf/10);
$(yAxis).css("display", "grid");



$(yAxis).css("gridTemplateColumns",2);
$(yAxis).css("gridTemplateRows", "repeat" + "(" + stuff2.yAxisInt +", " + 1 +"fr"+")");

let interval = max/stuff2.yAxisInt;

for(let i = 0; i < stuff2.yAxisInt; i++ ){
...
$(charTitle).css("width", stuff2.widthOf);
$(charTitle).html(stuff2.title);
$(charTitle).css("fontSize", (stuff2.fntSize + 'px'));
$(charTitle).css("color", stuff2.fntColor );
...
$(chart).css("height", stuff2.heightOf );
$(chart).css("width", stuff2.widthOf);
$(stuff3).append(chart);


//xAxis
let xAxis = $("<div></div>");
$(xAxis).css("height", stuff2.heightOf/10);
$(xAxis).css("width", stuff2.widthOf);


$(xAxis).css("gridTemplateColumns", stuff1.length);
...
$(xAxisLab).text(stuff1[i][2]);
$(xAxisLab).css("backgroundColor", stuff1[i][3]);
